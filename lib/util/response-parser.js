        ...
        for(var relation_name in relationships) {
          ...
          if(relation.data) {
            ...
            if(response.included) {
              for(var inc in response.included) {
                console.log("inspecting include type '%s' with id '%s' if it matches '%s' with id '%s'", inc.type, inc.id, rel_type, data.id);
                if( (inc.type === rel_type) && (inc.id === data.id) ) {
